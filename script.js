...
(function () {
  ...
  var FRAME_PATH   = 'https://8ispuxmgjxgu2r5q.public.blob.vercel-storage.com/templates/parrucchieri-002/frames/';
  var FRAME_COUNT  = 151;
  ...
  window.addEventListener('resize', function () { setupCanvas(); render(current / FRAME_COUNT); });
})();
...
(function () {
  ...
  if (!title) return;
  // Trigger reveal on next frame (after paint)
...
// ── Failsafe — elementos visíveis no carregamento ──
...
window.__imgFallback = function (img, label) {
  ...
  wrap.innerHTML =
    ...
    '<text x="400" y="310" text-anchor="middle" font-family="Cormorant Garamond, serif" font-style="italic" font-size="18" fill="currentColor" opacity="0.28">' + label + '</text>' +
    '</svg>';
